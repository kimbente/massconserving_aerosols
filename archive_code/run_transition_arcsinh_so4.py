...
# need features to initialize the model: so4 has 5 columns
...
x_train = x_train_so4_arcsinh.double().to(device)
y_train = y_delta_train_so4_arcsinh_unitvar.double().to(device)
...
for epoch in range(epochs):
    ...
    for batch_idx, (x_batch, y_batch) in enumerate(train_loader):
        ...
        if batch_idx % 1000 == 0:
            print(f'Epoch {epoch}, Batch {batch_idx}, Training RMSE Loss (transf.): {loss.item():.4f}')

        if batch_idx % 5000 == 0:
            model.eval()

            PRED_y_delta_val_arcssinh_unitvar = model(x_val_so4_arcsinh[val_row_indices].to(device)).detach().cpu()
